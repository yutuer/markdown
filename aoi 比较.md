...
3. 九宫格为什么不准
   ...
   4. 十字链表如果要求距离准确, 那么位置发生改变后, 就必须每次都重新筛选.  如果十字链表也不需要要求太准确, 那么也可以进行九宫格的类似优化
   
4. 技能判定.  

   1. aoi模块中最好传入一个 关系策略类(假定叫aoiRelationStrategy).  aoi用它来管理对象之间关系的建立, 删除. 每个对象上面也应该有个 管理和其他对象关系的类(bpAoiRelation).
   2. 对象可以在进行技能攻击的时候 , 使用bpAoiRelation来进行目标判定的遍历.(传入一个技能选择的策略, 返回一个对象组)